...
#define RELAY_FAN //isi pin relay kipas
...
void setup() {
  ...
  digitalWrite(RELAY_FAN, HIGH); // HIGH = relay OFF
...
void loop() {
  if (sensorMillis >= sensorInterval) {
    ...
    if (temperature > 24 && humidity > 70) {
      digitalWrite(RELAY_FAN, HIGH); // Kipas menyala
    } else {
      digitalWrite(RELAY_FAN, LOW); // Kipas mati
    }
